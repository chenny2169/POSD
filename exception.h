...
TEST(Exception, exception_OnlyVariable) {
...
TEST(Exception, exceptionMissingPeriodToken) {
  Scanner s("X=1");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Exception, exception_UnexpectedDisjTokenBeforePeriod) {
  Scanner s("X;.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Exception, exception_UnexpectedConjTokenBeforePeriod1) {
  Scanner s("X,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Exception, exception_UnexpectedConjTokenBeforePeriod2) {
  Scanner s("X=1,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Exception, exception_UnbalancedOperator1) {
...
TEST(Exception, exception_UnbalancedOperator2) {
